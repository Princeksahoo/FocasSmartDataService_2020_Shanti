...
namespace FocasSmartDataCollection
{
    public static class Utility
    {
        ...
        public static string ReadLastFile(string filePath, string programNo)
        {
            if (!Directory.Exists(filePath)) return string.Empty;
            int result = 0;
            IEnumerable<FileParts> finalFileList = null;
            List<FileParts> filePartList = new List<FileParts>();
            DirectoryInfo dirInfo = new DirectoryInfo(filePath);
            FileInfo[] reqfiles = dirInfo.GetFiles().Where(x => x.FullName.Contains(programNo)).ToArray();
            List<string> reqFileNames = reqfiles.Select(x => x.Name).ToList();
            foreach (string fname in reqFileNames)
            {
                string[] fnameParts = fname.Split('.');
                if (fnameParts.Count() >= 3)
                {
                    if (int.TryParse(fnameParts[1], out result))
                    {
                        filePartList.Add(new FileParts() { FileName = fname, fileIdentifier = result });
                    }
                }
                else
                {
                    filePartList.Add(new FileParts() { FileName = fname, fileIdentifier = 0 });
                }
            }
            if (filePartList.Count > 0)
            {
                finalFileList = filePartList.OrderByDescending(x => x.fileIdentifier);
            }
            return filePartList.Count > 0 ? finalFileList.First().FileName : "";
        }
